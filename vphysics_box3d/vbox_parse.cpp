...
static const Box3DKVSchemaProp_t kVehicleSteeringDescs[] = {
    ...
    { "ThrottleSteeringRestRateFactor", KVSCHEMA_DESC(vehicle_steeringparams_t, throttleSteeringRestRateFactor),
      FillFloatProp },
...
void Box3DPhysicsParseKV::ParseRagdollConstraint(
    constraint_ragdollparams_t* pConstraint, IVPhysicsKeyHandler* unknownKeyHandler)
{
    if (unknownKeyHandler)
...
void Box3DPhysicsParseKV::ParseCollisionRules(ragdollcollisionrules_t* pRules, IVPhysicsKeyHandler* unknownKeyHandler)
{
    ...
    ParseBox3DKVSchema(
        m_pCurrentBlock, kCollisionRulesDescs, ARRAYSIZE(kCollisionRulesDescs), &helper, pRules, unknownKeyHandler);
...
void Box3DPhysicsParseKV::ParseRagdollAnimatedFriction(
    ragdollanimatedfriction_t* pFriction, IVPhysicsKeyHandler* unknownKeyHandler)
{
    if (unknownKeyHandler)
    ...
    ParseBox3DKVSchema(
        m_pCurrentBlock, kRagdollAnimatedFrictionDescs, ARRAYSIZE(kRagdollAnimatedFrictionDescs), pFriction, pFriction,
        unknownKeyHandler);
...
IVPhysicsKeyParser* CreateVPhysicsKeyParser(const char* pKeyData, bool bIsPacked)
{
    ...
    if (!pszKV)
    {
        Log_Warning(
            LOG_VBox3D,
            "CreateVPhysicsKeyParser: Encountered invalid KV data. Falling back to a dummy KV. You may notice a broken "
            "prop/vehicle.\n");
